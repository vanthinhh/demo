...
*** Test Cases ***
# Positive Test Case - Đăng kí tài khoản thành công
DangKiTaiKhoanThanhCong
    ...
    Close Browser
# Negative Test Case - Đăng kí với email đã tồn tại
DangKiVoiEmailDaTonTai
    Open Browser    ${URL}    chrome
    Maximize Browser Window
    Set Selenium Speed    0.5
    Go To    ${URL}
    ...
    # Bước 3: Nhập thông tin cần thiết để đăng kí tài khoản mới
    ${random_string}=    Generate Random String    8    [NUMBERS]
    ${existing_email}=    Set Variable    testuser${random_string}@example.com
    
    Input Text    id=AccountFrm_firstname    YourFirstName
    Input Text    id=AccountFrm_lastname    YourLastName
    Input Text    id=AccountFrm_email    ${existing_email}
    Input Text    id=AccountFrm_telephone    0123456789
    Input Text    id=AccountFrm_fax    123456
    Input Text    id=AccountFrm_company    HoChiMinh
    Input Text    id=AccountFrm_address_1    YourAddress
    Input Text    id=AccountFrm_address_2    YourAddress
    Input Text    id=AccountFrm_city    YourCity
    ...
    # Bước 9: Kiểm tra thông báo lỗi
    Wait Until Element Is Visible    xpath=//*[@id="AccountFrm"]/div[1]/div
    Element Text Should Be    xpath=//*[@id="maincontainer"]/div/div/div/div[1]    Error: E-Mail Address is already registered!
